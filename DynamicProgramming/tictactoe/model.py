...
class State:
    ...
    @property
    def key(self) -> str:
        m_list = [str(m) for m in self._state]
        key = "".join(m_list).replace(" ", "_")
        return key

    @classmethod
    def replicate_from_Grid(
        cls, grid: Grid, my_mark: Mark, this_move: Tuple[int, int] = None
    ) -> State:
        new_state = cls(dimension=grid.N)

        new_state._state = [
            Memory.translate(mark=mark, myMark=my_mark) for mark in grid._grid
        ]

        if this_move is not None:
            exclude_index = grid._inx(position=this_move)
    ...
    def __str__(self) -> str:
        ...
        for r in range(self.N):
            ...
            for c in range(self.N):
                line += str(self._state[self._inx(position=(r + 1, c + 1))])
                line += "|"
            lines.append(line)
...
class Grid:
    ...
    def render(self) -> None:
        ...
        for r in range(self.N):
            line = "|"
            for c in range(self.N):
                line += str(self._grid[self._inx(position=(r + 1, c + 1))])
                line += "|"
            print(line)
        pass

    def get_value(self, position: Tuple[int, int]) -> Mark:
        return self._grid[self._inx(position=position)]
    ...
    def validate_new_place(self, position: Tuple[int, int]) -> bool:
        ...
        (row, column) = position
        logger.debug(
            f"validate new place: {row}:{type(row)}, {column}:{type(column)}, "
        )
        ...
        inx = self._inx(position=position)
    ...
    def update(self, position: Tuple[int, int], mark: Mark) -> None:
        if self.validate_new_place(position=position):
            # Update the grid with the mark
            self._grid[self._inx(position=position)] = mark
        else:
...
class Place_Interface(Protocol):
    ...
    def get_scene_key(self, grid: Grid, mark: Mark) -> str:
        state = State.replicate_from_Grid(grid=grid, my_mark=mark)
        return state.key
...
class Manual_Console_Impl(Place_Interface):
    ...
    def place_mark(self, grid: Grid, mark: Mark) -> Tuple[int, int]:
        ...
        print(f"{self.name} - {str(mark)} input now")
...
class AI_Impl(Place_Interface):
    ...
    def place_mark(self, grid: Grid, mark: Mark) -> Tuple[int, int]:
        ...
        print(f"{self.name} - {str(mark)} input now")
        print(f"{self.name} - Current state: {scene_key}")
        return self.brain.pick_space(scene_key=scene_key)
