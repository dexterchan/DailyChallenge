...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--statisticfile", type=str, default="/tmp/game.statistics.json"
    )
    ...
    while iteration <= max_iteration:
        ...
        min_probability_try_new = (
            min_probability
            + (1 - min_probability) * math.exp(-1 * iteration * learning_rate)
            if not human_player
            else 0.1
        )
        ...
        A_brain = (
            Manual_Console_Impl(name="A")
            if human_player
            else AI_Impl(name="epA", brain=ep_ai1)
        )

        B_brain = AI_Impl(name="epB", brain=ep_ai2)
        ...
        tic_tac_toe_game: Tic_Tac_Toe_Game = Tic_Tac_Toe_Game(
            dimension=DIM, agentA=agentA, agentB=agentB
        )
