...
# there is status in the function PLUS, MINUS
# at start status is PLUS
# if prevalue is None, status is PLUS
# when curvalue  > prevalue, status is PLUS, keep "add" curvalue to buffer
# when curvalue < prevalye, status is MINUS, keep minus curvalue from buffer
# when curvalue = prevalye , keep status
        ...
        for inx in range(len(s)-1, -1, -1):
            ...
            if preValue is not None:
                if curValue > preValue:
                    status = STATE.PLUS
                elif curValue < preValue:
                    status = STATE.MINUS
            preValue = curValue
            change = curValue if status == STATE.PLUS else curValue * (-1)
            result += change
...
if __name__ == "__main__":
    ...
    print(Solution().romanToInt('MMCMX'))
    # 2910

    print(Solution().romanToInt('MMCCMXIIX'))
    # 2910